...
class TextFieldStreamer extends InputStream implements KeyListener {
	...
	public void keyPressed(KeyEvent e) {
		if (e.getKeyCode() == KeyEvent.VK_ENTER) {
			...
			//Sends input immediatly to Input class for processing.
			 try {
			...
			} catch (InterruptedException | IOException userInputIEOrIO) {
				//error & terminate output start
				System.err.print("The user was not found or character files failed to load.\n\n");
				try {
					Thread.sleep(3000);
				...
				userInputIEOrIO.printStackTrace();
				System.out.print("\nTerminating...\n");
				try {
					Thread.sleep(10000);
				} catch (InterruptedException e1) {
